  ...
  <div>
    <div>
      <div class="d-flex justify-content-center">
        <h3>1년전 개봉작 {{ yearAgoMovies.length }} 개</h3>
      </div>
      <br>
      <br>
      <div class="swiper d-flex justify-content-center" style="display:flex; white-space:nowrap; ">
        <div
          id="movie-poster"
          style="transition: all 0.2s linear; overflow: hidden;"
          v-for="yearAgoMovie in yearAgoMovies"
          :key="yearAgoMovie.id"
        >
          <div class="card mx-3" style="width: 20rem;">
            <img :src="yearAgoMovie.poster_path" class="card-img-top " alt="..."
              @click="getMovieDetail(yearAgoMovie.id)" type="button" data-toggle="modal" data-target="#exampleModal"
              height="475"
            >
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    yearAgoMovies: function() {
      return this.$store.state.main.yearAgoMovies
    },
  },

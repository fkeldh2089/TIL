# 21610 마법사 상어와 비바라기
import sys
sys.stdin = open('input_21610.txt')

# N은 필드 크기, M은 구름의 이동
N, M = list(map(int, input().split()))
field = [list(map(int, input().split())) for _ in range(N)]
Move = [list(map(int, input().split())) for _ in range(M)]
dir_r = [0, -1, -1, -1, 0, 1, 1, 1]
dir_c = [-1, -1, 0, 1, 1, 1, 0, -1]
now = [[N-1, 0], [N-1, 1], [N-2, 0], [N-2, 1]]  # 구름의 초기 좌표
mkc = []  # 다음 구름 후보 좌표
water = 0
for p1 in range(N):
    for p2 in range(N):
        water += field[p1][p2]  # 전체 물
        if field[p1][p2] >= 2:  # 물이 2이상인 부분 좌표
            mkc.append([p1, p2])

for p in range(M):  # 이동 횟수만큼 반복하는 비바라기
    cloud_count = len(now)  # 구름의 수
    clouds = []
    for q in range(cloud_count):  # 구름 수 만큼 반복
        pos = now[q]
        d, s = Move[p]
        new_pos = [pos[0] + s * dir_r[d-1], pos[1] + s * dir_c[d-1]]
        if not (0 <= new_pos[0] < N) or not (0 <= new_pos[1] < N):
            if new_pos[0] <= -1:
                new_pos[0] %= N
            if new_pos[1] <= -1:
                new_pos[1] %= N
            if new_pos[0] >= N:
                new_pos[0] %= N
            if new_pos[1] >= N:
                new_pos[1] %= N
        field[new_pos[0]][new_pos[1]] += 1
        water += 1
        clouds.append(new_pos)

    for q in range(cloud_count):
        pos = clouds[q]
        for r in range(1, 8, 2):  # 대각선 4방향에 대하여
            new_pos = [pos[0] + dir_r[r], pos[1] + dir_c[r]]
            if (0 <= new_pos[0] < N) and (0 <= new_pos[1] < N):  # 대각선이 필드안에 있다면
                if field[new_pos[0]][new_pos[1]] >= 1:  # 물이 있다면
                    field[pos[0]][pos[1]] += 1  # 물이 증가한다
                    water += 1
        if field[clouds[q][0]][clouds[q][1]] >= 2 and not(clouds[q] in mkc):
            mkc.append(clouds[q])

    now = []  # now 초기화
    dry = []  # 물이 적어진 곳
    for q in range(len(mkc)):
        if not(mkc[q] in clouds):
            now.append(mkc[q])
            field[mkc[q][0]][mkc[q][1]] -= 2
            water -= 2
            if field[mkc[q][0]][mkc[q][1]] < 2:  # 2보다 물이 적어졌으면
                dry.append(mkc[q])
    for q in range(len(dry)):
        mkc.remove(dry[q])

print(water)